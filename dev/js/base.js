//@codekit-prepend "modernizr.custom.min.js";
...
// Google Fonts
WebFontConfig = {
  google: { families:[  'Open+Sans:400,400:italic,600,700:latin', 'Merriweather:400,700:latin' ] }
};
(function() {
...
$(document).ready(function(){
  ...
  $('a[href="#nav"]').on('click', function(e){
    navOverlay.fadeIn();
    navOverlay.find('.nav__overlay__top').addClass('animated fadeIn');
    navOverlay.find('.nav__overlay__body').addClass('animated bounceInUp');
    $('body').addClass('nav__overlay--active');
    e.preventDefault();
  });

  $('.nav__overlay__close').on('click', function(e){
    navOverlay.fadeOut();
  ...
  $(document).keyup(function(e) {
    if (e.keyCode === 27 && $('body').hasClass('nav__overlay--active')) { // esc
      $('.nav__overlay__close').click();
    }
  });

  navOverlay.on('click',function() {
    if ($('body').hasClass('nav__overlay--active')) {
      $('.nav__overlay__close').click();
    }
  });
  ...
  if ($("#mc_embed_signup").length) {
    ...
    var mcapikey = $('#mc-embedded-subscribe-form').data('api'),
        mcid = $('#mc-embedded-subscribe-form').data('id'),
        mcus = $('#mc-embedded-subscribe-form').data('us');
    ...
    $("#mc-embedded-subscribe-form").submit(function(e){
      if ($("#mce-EMAIL").val() === "") {
        ...
        data = {
          ajax            : 'true',
          apikey          : mcapikey, //"b43fd3fb4d6ffc1b3470593e3e6a7b16-us5",
          id              : mcid,     //"2e2b9463f9",
          ...
          merge_vars      : {
            'GROUPINGS' : [{name: 'Source', 'groups' : 'Homepage'}]
          }
        };
        $.ajax({
          type: "POST",
          url: "//"+mcus+".api.mailchimp.com/1.3/?method=listSubscribe",
          data: data,
          dataType: 'jsonp'
        }).complete(function(){
          // iMoneyTracker.post({
          ...
          // });
          $('#mce-EMAIL').val('');
          $('#mc-embedded-subscribe').val('Done!').addClass('btn-success');
        });
      }
      e.preventDefault();
    });
  ...
  if($('body').hasClass('single')) {
    ...
    $.getJSON('http://graph.facebook.com/?id=https://www.imoney.my/articles' + url, function(data){
      fbShareCount = data.shares;
      if (fbShareCount > 0 ) {
        $('.share__fb').find('a').append('<span>' + fbShareCount + '</span>');
        totalShares+= fbShareCount;
        if (totalShares > 0) {
          $('.share__total').text(totalShares + ' shares');
        }

      }
    });

    $.getJSON('https://cdn.api.twitter.com/1/urls/count.json?url=https://www.imoney.my/articles' + url + '&callback=?', function(data){
      twShareCount = data.count;
      if (twShareCount > 0 ) {
        $('.share__tw').find('a').append('<span>' + twShareCount + '</span>');
        totalShares+= twShareCount;
        if (totalShares > 0) {
          $('.share__total').text(totalShares + ' shares');
        }
      }
